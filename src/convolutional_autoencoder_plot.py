...
def comparison(test_images, test_labels, DecodedImages, EncodedClassifier, encoder_model, option):
    ...
    for i in range(n):
        ...
        plt.imshow(DecodedImages[index, 0, :, :], cmap='gray', vmin=-1, vmax=1)

        # Original-Testbild holen, normalisieren, reshapen → dann ENCODEN
        original_image = (test_images[index].astype("float32") / 255.0) * 2 - 1
        original_image = torch.tensor(original_image.reshape(1, 1, 28, 28), dtype=torch.float32).to(device)
...
def compute_average_classwise_errors(original_images, reconstructed_images, labels, image_shape=(28, 28)):
    ...
    if reconstructed_images.ndim == 4:
...
def save_decoded_images(decoded_images, dataset_name, category='Decoded Images'):
    ...
    for i, img in enumerate(decoded_images):
        ...
        if img.ndim == 3 and img.shape[0] == 1:
            img = img.squeeze(0)  # entfernt die Kanaldimension vorne
...
def visualize_single_reconstruction_torch(original_tensor, reconstructed_tensor):
    original = to_numpy(original_tensor.squeeze())
    reconstructed = to_numpy(reconstructed_tensor.squeeze())

    abs_diff = np.abs(original - reconstructed)
    ssim_score, ssim_map = ssim(original, reconstructed, full=True, data_range=2)

    fig, axs = plt.subplots(1, 4, figsize=(12, 3))
    axs[0].imshow(original.reshape(28,28), cmap='gray')
    axs[0].set_title("Original")
    axs[1].imshow(reconstructed.reshape(28,28), cmap='gray')
    axs[1].set_title("Rekonstruktion")
    axs[2].imshow(abs_diff.reshape(28,28), cmap='hot')
    axs[2].set_title("Pixel-Differenz")
    axs[3].imshow((1 - ssim_map).reshape(28, 28), cmap='hot')
    axs[3].set_title("SSIM-Fehler")
    for ax in axs:
        ax.axis('off')
    plt.suptitle(f"SSIM Score: {ssim_score:.4f}")
    plt.tight_layout()
    plt.show()

def analyze_reconstruction_dataset(originals, reconstructions):
    # original/recon: shape (N, 1, H, W)
    originals_np = to_numpy(originals.squeeze(1))   # (N, H, W)
    recon_np = to_numpy(reconstructions.squeeze(1))

    abs_diff = np.abs(originals_np - recon_np)
    mean_abs_error = np.mean(abs_diff, axis=0)

    # Durchschnittlicher Fehler als Heatmap
    plt.figure(figsize=(4, 4))
    plt.imshow(mean_abs_error, cmap='hot')
    plt.title("⌀ Absoluter Fehler pro Pixel")
    plt.colorbar()
    plt.axis('off')
    plt.show()
    ...
    ssim_scores = []
    mse_scores = []
    for orig, rec in zip(originals_np, recon_np):
        ssim_score, _ = ssim(orig, rec, full=True, data_range=2)
        ssim_scores.append(ssim_score)
        mse_scores.append(np.mean((orig - rec) ** 2))

    plt.figure(figsize=(10, 4))

    plt.subplot(1, 2, 1)
    plt.hist(ssim_scores, bins=30, color='skyblue')
    plt.title("SSIM Score Verteilung")
    plt.xlabel("SSIM")
    plt.ylabel("Häufigkeit")

    plt.subplot(1, 2, 2)
    plt.hist(mse_scores, bins=30, color='salmon')
    plt.title("MSE Verteilung")
    plt.xlabel("MSE")
    plt.ylabel("Häufigkeit")
    ...
    print(f"⌀ SSIM: {np.mean(ssim_scores):.4f}, ⌀ MSE: {np.mean(mse_scores):.6f}")
#
#def save_trained_model(autoencoder, encoder, option):
#    if option == 0:
    #if option == 0:
    #    save_plot(fig, dataset_name='MNIST', category='Classifier', filename='autoencoder_classifier_comparison.png')
    #else:
    #    save_plot(fig, dataset_name='FashionMNIST', category='Classifier', filename='autoencoder_classifier_comparison.png')
#torch.save(trainer.autoencoder_model.state_dict(), os.path.join(SAVE_DIR, "Models", "autoencoder_model.pth"))
#torch.save(trainer.encoder_model.state_dict(), os.path.join(SAVE_DIR, "Models", "encoder_model.pth"))
#torch.save(results["baseline_classifier"].state_dict(), os.path.join(SAVE_DIR, "Classifier", "baseline_classifier.pth"))
#torch.save(results["encoded_classifier"].state_dict(), os.path.join(SAVE_DIR, "Classifier", "encoded_classifier.pth"))
#    else:

def visualize_encoded_clusters(self, encoded_data, labels, original_images, use_images=False, method="tsne"):
    ...
    reduced = reducer.fit_transform(encoded_data)

    fig, ax = plt.subplots(figsize=(16, 14))

    if use_images:
        ax.scatter(reduced[:, 0], reduced[:, 1], s=0.01, alpha=0.0)
        for i in range(reduced.shape[0]):
            image = original_images[i]
            if image.shape != (28, 28):
                image = image.reshape(28, 28)

            imagebox = OffsetImage(image, zoom=0.4, cmap='gray')
            ab = AnnotationBbox(imagebox, reduced[i], frameon=False)
            ax.add_artist(ab)

        ax.set_title("t-SNE Visualisierung mit Originalbildern")
    else:
        scatter = ax.scatter(reduced[:, 0], reduced[:, 1], c=labels, cmap='tab10', s=15, alpha=0.8)
        ax.set_title("t-SNE Cluster der Encoded Features (mit Farblegende)")
